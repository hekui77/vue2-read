...
// 两件事：
//  数据响应式的入口：分别处理 props、methods、computed、watch
...
export function initState (vm: Component) {
  ...
  // 处理 methods 对象，校验每个属性值是否为函数，和 props 属性对比 进行判重处理，最后得到 vm[key] = methods[key]
  ...
  // 做了三件事
  //  1、为 computed[key] 创建 watcher 实例，默认懒执行
  //  2、代理 computed[key] 到 vm 实例
  //  3、判重，computed 中的 key 不能和 data、props 中的属性重复
  if (opts.computed) initComputed(vm, opts.computed)
  // 做了三件事
  //  1、处理 wath 对象
  //  2、为每个 wath[key] 创建 watcher 实例，key 和 watcher 实例可能是一对多的关系
...
// 处理 props 对象，为 props 对象的每个属性设置响应式，并将其代理待 vm 实例上
function initProps (vm: Component, propsOptions: Object) {
  const propsData = vm.$options.propsData || {}
  const props = vm._props = {}
  // 缓存 props 的每一个 key，性能优化
  // cache prop keys so that future props updates can iterate using Array 缓存 prop 键，以便将来的 props 更新可以使用 Array 进行迭代
...
/* 
  三件事
    1、为 computed[key] 创建 watcher 实例，模式是懒执行
    2、代理 computed[key] 到 vm 实例
    3、判重，computed 中的 key 不能和 data、props 中的属性重复
  computed: {
    key1: function() { return xxx },
    key2: {
...
function initComputed (vm: Component, computed: Object) {
  ...
  for (const key in computed) {
    ...
    if (!(key in vm)) {
      ...
      // 这样就可以使用 vm.computed[key] 访问计算属性了
...
// 返回一个函数，这个函数在访问 vm.computedProperty 时会被执行，然后返回执行结果
function createComputedGetter (key) {
  // computed 属性值会缓存的原理也是在这结合 watcher.dirty、watche.evaluate、watcher.update 实现的
  return function computedGetter () {
    // 得到当前的 key 对象的 watcher
    const watcher = this._computedWatchers && this._computedWatchers[key]
    if (watcher) {
      // 计算 key 对应的值，通过执行 computed.key 的回调函数来得到
      // watcher.dirty 属性就是常说的 computed 计算结果会缓存的原理
      // <template>
